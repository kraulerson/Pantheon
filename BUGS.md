...
| 14 | SEV-4 | Open | test-suite (gitea live integration) | `test/gitea-live.integration.test.ts` FAILS (not skips) since the 2026-08-17 token rotation: the new minimal-scope token is `write:repository,read:user` but the test needs `write:user` → `Gitea API 403`. The test is meant to be guarded — it runs whenever a token is present, so a correctly-scoped-down token turns a skip into a red suite. Unrelated to #12/#13; noted while verifying them. Fix by having the guard check scope (preferred) rather than widening the token and undoing the minimal-scope decision | ad hoc (2026-08-19) | Defer | — | — |
