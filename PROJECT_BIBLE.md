...
### ADR-0009 — Session waker: promotion to product behind deterministic guardrails (TP-1, TP-5 partial, XC-6)
**Status:** Accepted · 2026-08-31 (M1 task 4). The `cli-channel-loop` spike becomes product in two stages: the decision layer first (`src/waker/` — deny-by-default directional allowlist, per-pair sliding-window rate cap, light-context wake, hold-until-idle dispatcher), the channel runner second (research-preview protocol + live bridge, acceptance by live smoke test). Every wake passes one gate in order: configured -> allowlisted -> rate cap -> idle. No model call takes part in an admissibility decision (CC3). WAKE-NOT-BODY and idle-only delivery are preserved; TP-5's cadence-backoff lever is not implemented (ruling: wake when needed). Full record: `docs/ADR documentation/ADR-0009-session-waker-promotion-and-deterministic-guardrails.md`.
...
| **TM-024** | One browser origin for chat + console (ruling 2026-08-27, accepted residual): script on the LibreChat page (XSS, inline-served upload, self-hosted artifact sandbox) reaches the console — terminals included — with the operator session | SEV-2 (accepted) | Cookie scoped to `/harness`; Sec-Fetch-Site check incl. WebSocket handshakes; `frame-ancestors 'self'` + frame-bust; `ALLOW_REGISTRATION=false`; `SANDPACK_BUNDLER_URL` never on a household host; uploads never inline; BUGS #36 step-up gates mint/config when it lands. Origin separation impossible today (edge Host rewrite; cross-site embed breaks LibreChat sign-in) | Accepted (APPROVAL_LOG 2026-08-27) |
...
3. **Versioned & reversible.** Every persistent entity is created/mutated only through versioned, reversible migrations. Mutable entities carry `version`, `createdAt`, `updatedAt`. Deprovision is a `status` transition, never a hard delete of audit rows; AuditEntry and ApprovalRecord are append-only. **Exception (single-operator local SQLite — ruling A, 2026-08-25, APPROVAL_LOG):** the control-plane's own SQLite tables (registry, session, keycard, and later tables of the same kind) use **forward-only additive DDL** — `CREATE TABLE IF NOT EXISTS` and additive `ALTER TABLE ADD COLUMN` — applied at startup. Any **destructive** change (drop, rename, retype, or narrowing a constraint) requires a numbered migration script plus a backup taken first, never an in-place edit. Purely operational counters (`useCount`, `denyCount`, `lastUsedAt`) are exempt from the `version`/`updatedAt` requirement; state transitions (`revokedAt`, `provisioned`) are recorded by timestamp and `updatedAt`. Peta-side stores (identity, approvals, audit) are unaffected by this exception.
...
**ApprovalSource (config, not persisted; BUGS #42, 2026-08-28)** — the approval stores the inbox and the keycard door read: this host's Peta (label `Pantheon`) plus each `PANTHEON_APPROVAL_SOURCES` entry `{ label, url, token }` (the household's identities run behind Alden's capability gateway, which has its own Peta). Read-only, reference-only (D8), all stores in parallel; a store that fails is named, never silently dropped; a malformed list stops the service at startup.
...
**XC-6 — inbound adapters are fail-closed (2026-08-31, ADR-0009).** Every adapter that can deliver
something INTO a session (the waker first; any future inbound relay) must refuse to dispatch at all
until its allowlist is configured, and must deny by default once it is: an absent or empty allowlist
is never "allow everything", and each adapter carries a negative test proving the refusal.

**As built 2026-08-27 (harness under the chat address; design `docs/2026-08-27-harness-under-chat-address-design.md`).** The operator session cookie is scoped to the console's mount (`Path=/harness` under the chat address, `/` at the root) so LibreChat's backend never receives it; the Sec-Fetch-Site CSRF check also refuses `Upgrade: websocket` handshakes the browser labels same-site/cross-site (closes the sibling-host half of BUGS #26); console HTML sends `Content-Security-Policy: frame-ancestors 'self'` and busts any same-origin frame to the top window. The one-origin residual (TM-024) is accepted by ruling. Two sign-ins remain (console passphrase, LibreChat account) until M2.
...
- **Chat-page realisation (ruling 2026-08-26, "both modes"):** on the LibreChat page the identity-vs-bare choice is LibreChat's own endpoint picker — **"Pantheon"** (identity route through the Facade; live when M2 lands) beside **"Basic LLM - <brain>"** entries (one per LAN llama.cpp server: no persona, no memory, no tools, no Peta, nothing to gate). Both stay permanently; the operator switches per conversation. The Basic entries are deploy config (`deploy/librechat.yaml`), not harness code. **Thinking is a per-conversation choice on every entry (ruling 2026-08-26):** today the brains honour only on/off (`chat_template_kwargs.enable_thinking`), so each raw brain has a "fast" and a "thinking" picker entry; the M2 Facade turns LibreChat's Reasoning-effort dropdown into the per-brain control for identity AND bare sessions (skeleton step 05 requirement).
...
**Pending-Approvals inbox (M1 task 3, TP-2; built 2026-08-26, multi-source 2026-08-28).** `GET /admin/approvals` (linked **Approvals** in the harness chrome): one table of every request the household's approval stores report as PENDING — **Source** · Identity · Tool · Target · Age in words · Status · Ref — reference-only (D8), read-only until M2 C.3 (the page holds `listApprovals`-only readers; the decide verb is structurally unreachable).
- **Empty:** "No pending approvals — nothing is waiting on you. Checked: <stores>."
- **Loading:** none — server-rendered; **Reload** link.
- **Error:** a store that did not answer → "[!] <store>: <our label> — its requests are missing from this page." above the others' rows (200); every store failed → 502 "[!] Could not read any approval queue"; no store configured → 503 "[!] the approval gate (Peta) is not configured on this server". Upstream text is never echoed.
- **Success:** rows; "More approvals are waiting than this page shows" when a walk stopped early; resolved / unidentified items counted, never listed.

**The harness under the chat address (ruling 2026-08-27; Feature 11).** The console is served at `https://pantheon.ferrumcorde.com/harness/…` (VM Caddy `handle_path /harness/*` → admin service with `X-Forwarded-Prefix: /harness`; the service builds every link/redirect/asset/socket URL from that validated prefix; the admin address stays a root mount). LibreChat's footer reads **Terminals | Configuration**; Help opens `/harness/help`. Every console page shares `/assets/harness.css` — LibreChat v0.8.7's own token values — and follows LibreChat's `color-theme` (dark/light/system); terminals stay on the dark surface in both themes. The harness's **Chat** tab embeds the chat page (same-origin iframe) under the chat address and is a labelled link on the admin address. Registration stays on the Configuration page. Resolves step-08 item 5 for the terminal plane; the chat half (starting a persona conversation from the harness) waits for M2.
...
- **As built 2026-08-27:** the chat site (`pantheon.ferrumcorde.com`, `:443`) path-splits — `/harness/*` → admin service (`host.docker.internal:8088`, prefix stripped, `X-Forwarded-Prefix: /harness` SET), everything else → LibreChat; `X-Frame-Options: SAMEORIGIN` there (lets the harness embed chat). The admin site (`:8443`) stays a root mount, `DENY`, and deletes any client-supplied prefix header. The Caddyfile is a bind-mounted FILE: after `git pull` recreate the container (`docker compose up -d --force-recreate caddy`) — `caddy reload` re-reads the old inode (2026-08-27).
...
### Context health check — 2026-08-28 (features 9–12)

**Built since the last check:** `tmux-aware-launcher` (F8), `scoped-session-keycard` (F9, ADR-0008), `pending-approvals-inbox` (F10, multi-source after UAT-4), `harness-under-chat-address` (F11) + fixes #39 (terminal fit) and #42 (approval stores). UAT-3 (22/23) and UAT-4 (16/20; #14 → #42 fixed; #16/#19 not reproducible) closed. **Remaining (M1):** machines sidebar (next; Karl 2026-08-28), session-waker promotion + guardrails (task 4), cross-project task board (task 5), `pantheon doctor` (folded into M2 acceptance). **Data model:** unchanged tables (registry, session, keycard — additive-DDL ruling A); ApprovalSource is env config. **Known issues:** BUGS #17, #24–#30, #34–#38, #40, #41 open (SEV-3/4); TM-024 accepted; Alden gateway approval source pending a token from the Alden infra side (`docs/handoffs/2026-08-28-prompt-for-alden-infra-approval-source.md`). Bible sections 4, 5, 7, 9, 11 updated in this check.
