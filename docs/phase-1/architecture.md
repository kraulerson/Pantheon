...
- **D (Bifrost instead of Peta):** rejected as primary because it lacks the durable HITL approval gate; choosing it forces us to rebuild the write-approval boundary in our own code (Option B's risk, narrowly). **NO LONGER THE FALLBACK (corrected 2026-07-09):** the 2026-07-02 landscape re-validation downgraded it (HITL *explicitly disclaimed in gateway mode*); the ladder is now **Peta → ToolHive (+ control-plane approval) → Preloop → small custom gateway**. **Non-promotion rule:** a brain-plane Bifrost (Alden Phase 3) runs with **zero MCP clients registered**; promoting it to the tool gateway would enable the parallel tool-governance surface that wall exists to exclude. Never promote without re-ratification.
