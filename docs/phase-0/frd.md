...
| M15 | **Configuration / Service Registry page (#11 GUI, promoted — D10)** | If the operator, having cleared the strongest auth tier (step-up/passkey, D6) on the separate admin surface, submits a create/edit/remove against an AI backend endpoint (BackendRegistry), an MCP server registration (Peta admin REST API), or a control-plane service endpoint (Qdrant/Gitea/Bridge/Obsidian/Peta), the system must validate the full change and only if valid apply it atomically and output the updated state. The page is reachable ONLY from the privileged admin surface and **never from any session or session tool**. | **Error:** access from a non-privileged tier or from a session is denied (page never renders); an invalid/malformed change is rejected with a specific error and **no partial write persists** (fail closed); an attempt to rebind an existing identity is rejected ("identity↔backend binding is immutable, #14a"). **Recovery:** operator corrects and resubmits; backend `endpoint` is editable in place, identity rebind requires deprovision + re-provision. Controls are shape/label/icon-based, never color-only (CC1). | Promoted from S3 (GUI) + M12 (API-level) into one MVP component per D10 (2026-06-13). It is the GUI over the TM-002 (backend-rebind) and TM-011 (management-from-session) surfaces, so it inherits both SEV-1 mitigations directly. |
...
| S3 | **Gateway management GUI (#11 GUI)** | ~~A UI over the admin REST API for add/remove/edit MCP server registrations, behind the strongest auth tier.~~ **Promoted to MVP and folded into M15 (Configuration / Service Registry page) per D10 (2026-06-13).** | ~~MVP drives #11 via the admin API directly (M12)...~~ No longer deferred — see M15 above. |
