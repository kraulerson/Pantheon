...
- **M15 — Configuration / Service Registry page (#11 GUI, promoted — Resolved Decision D10):** If the operator, having cleared the strongest auth tier (step-up/passkey, D6) on the separate admin surface, opens the Configuration page and submits a create/edit/remove against (a) an **AI backend endpoint** (BackendRegistry: endpoint, kind, displayName, enabled), (b) an **MCP server registration** (via Peta's admin REST API), or (c) a **control-plane service endpoint** (Qdrant, Gitea, Alden Bridge, Obsidian vault, Peta URL), the system must validate the full change and, only if valid, apply it atomically and output the updated state. The page is reachable ONLY from the privileged admin surface and is **never reachable from any session or session tool** (closes the TM-002 backend-rebind and TM-011 management-from-session surface). Editing a backend's endpoint is permitted; **rebinding an existing identity to a different backend is not** (immutable identity↔backend binding, #14a). Failure state: access from a non-privileged tier or from a session is denied; an invalid/malformed change is rejected with a specific error and **no partial write persists** (fail closed); controls are shape/label/icon-based, never color-only (CC1).
- **M16 — Claude-CLI session = persistent web SSH-terminal tab (new modality — Resolved Decision D11, ADR-0005):** If the operator opens a "Claude CLI" session against a selected dev machine, the system must open a **new tab hosting a persistent, direct SSH terminal** (xterm.js) to that machine — a distinct session MODALITY from the LibreChat chat pane — so the operator can run Claude Code there. The harness frame (one entry point behind the #9 auth gate) presents both modalities; the New Session popup routes "Claude CLI → dev machine" to a terminal tab. The dev machine is referenced by a stable **logical name** (its IP is editable config); the dev-machine SSH key lives in **vault custody only and never enters session/terminal context** (#14b). **Logic trigger:** operator selects a `claude_cli` identity/backend + a DevMachine and confirms → control-plane resolves the key from custody and brokers the SSH connection server-side. **Failure state:** if the machine is unreachable or the key cannot be resolved, the tab fails closed with a specific text error and **no terminal opens**; access from outside the #9-authenticated harness is denied (TM-020). Controls/states are shape/label/icon-based, never color-only (CC1).
...
- **Gateway management GUI (#11 GUI):** ~~A UI over the admin REST API for MCP server registration, behind the privileged tier.~~ **Now folded into M15 (Configuration / Service Registry page) and promoted to MVP per Resolved Decision D10 (2026-06-13).** No longer a Should-Have; see M15 above and §5.
...
- **Configuration / Service Registry page (M15 — #11 GUI promoted, Resolved Decision D10):** privileged-only admin page; CRUD AI backend endpoints, MCP server registrations, and control-plane service endpoints; never session-reachable; validate + fail-closed; immutable identity↔backend binding
- **Claude-CLI session = persistent web SSH-terminal tab (M16 — new modality, Resolved Decision D11, ADR-0005):** harness frame behind #9 auth presents chat + xterm.js SSH-terminal modalities; New Session routes "Claude CLI → dev machine" to a terminal tab; dev machine referenced by logical name; SSH key in vault custody only
...
- **D10 — Scope promotion: Configuration / Service Registry page (M15) is now MVP.** What was #11 at API-level only (M12, the admin REST API) plus the Should-Have S3 "gateway management GUI" is promoted into a single required MVP component — one privileged-only admin Configuration page that CRUDs AI backend endpoints (BackendRegistry), MCP server registrations (over Peta's admin REST API), and the control-plane's own service endpoints (Qdrant, Gitea, Alden Bridge, Obsidian vault, Peta URL). Promoted per operator on 2026-06-13. Rationale: at one operator the endpoints are reconfigured often enough that an API-only surface is real friction, and a single audited config page is safer than editing config out of band; it inherits D6 step-up, the never-from-session constraint (TM-011), validate-and-fail-closed writes, and the immutable identity↔backend binding (#14a — endpoints editable, existing identities not rebindable).
- **D11 — Claude-CLI sessions are a second UI modality: persistent web SSH-terminal tabs, not a chat backend (M16, ADR-0005).** A "Claude CLI" session opens a new tab with a persistent, direct xterm.js SSH terminal to a selected dev machine (the operator runs Claude Code there); it is distinct from the LibreChat chat pane. The UI plane therefore hosts two modalities behind one harness frame (single entry point behind the #9 auth gate). Identities of `kind: claude_cli` bind to the `claude_cli` backend and reference a dev machine **by stable logical name** — so editing a machine's IP never breaks the immutable identity↔backend binding (#14a). The dev-machine SSH key lives in vault custody only, never in session/terminal context; the terminal is a distinct trust surface (TM-020). Decided per operator on 2026-06-13. Rationale: the operator's Claude Code workflow is interactive operator-driven terminal I/O on dev boxes, not recalled-content grounding — modeling it as a chat backend would wrongly drag it under the taint pipeline and mismodel a shell as a turn-based chat; binding by logical name keeps IPs editable config without churning identity bindings.
...
- **Gitea base URL + repo-layout convention — RESOLVED (2026-06-13):** the control-plane talks to Gitea **directly** at `https://gitea.ferrumcorde.com` (via Caddy; IP fallback `http://10.100.23.76:3000`), authenticating with an admin token supplied at runtime via the `GITEA_TOKEN` env var loaded from a **gitignored** `.env.local` (never committed). Persona-load + identity-repo provisioning use this privileged direct client; session-driven Gitea writes remain subject to the trust/approval gate (D2/D4). Security follow-up: the token was transcript-exposed once and must be **rotated**, and long-term it belongs in gateway/vault custody alongside other secrets, referenced by handle (Bible §7). The per-identity repo/scope naming/layout follows the existing Alden Gitea convention (see "Importability" below). No longer open. (See APPROVAL_LOG Scope Changes, 2026-06-13.)
